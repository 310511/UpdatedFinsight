...
Return ONLY JSON."""
    ...
    try:
        ...
        reports["cash_flow_statement"] = json.loads(res.choices[0].message.content.strip())
...
Return ONLY JSON."""
    ...
    try:
        ...
        reports["anomaly_suspicious_transaction_report"] = json.loads(res.choices[0].message.content.strip())
    except Exception as e:
...
Return ONLY JSON."""
    ...
    for report_name, prompt in prompts.items():
        try:
            ...
            reports[report_name] = json.loads(res.choices[0].message.content.strip())
        except Exception as e:
...
Return ONLY valid JSON with REAL extracted data. No explanations or markdown formatting."""
    ...
    try:
        ...
        gst_report = json.loads(res.choices[0].message.content.strip())
...
Return as a single comprehensive text analysis (not JSON)."""
    ...
    try:
        ...
        analysis_text = analysis_res.choices[0].message.content.strip()
    except Exception as e:
...
Return ONLY JSON."""
    ...
    try:
        ...
        audit_summary = json.loads(res.choices[0].message.content.strip())
    except:
        audit_summary = {"error": "Could not generate audit summary"}
...
Return ONLY valid JSON. No explanations or markdown formatting."""
    ...
    try:
        ...
        audit_report = json.loads(res.choices[0].message.content.strip())
...
Return ONLY JSON."""
    ...
    try:
        ...
        contract_data = json.loads(res.choices[0].message.content.strip())
    except:
        contract_data = {"error": "Could not extract contract data"}
