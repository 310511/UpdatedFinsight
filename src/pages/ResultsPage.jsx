    ...
    <div className="min-h-screen bg-gray-50 flex flex-col p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        {finalInsights.map((card, index) => (
          <div
            key={index}
            className={`${card.bg} p-4 rounded-lg shadow-sm border border-gray-100 flex flex-col items-start hover:shadow-md transition-shadow`}
          >
            ...
            <div className="space-y-6">
              ...
              {result.header && (
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  <h2 className="text-2xl font-bold text-gray-900 mb-4">{result.header.title || "GST Compliance Audit Report"}</h2>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    ...
                    {result.header.prepared_by && (
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{result.header.prepared_by}</p>
                      </div>
                    )}
                    {result.header.date && (
                      <div>
                        <span className="text-gray-600">Date:</span>
                        <p className="font-semibold text-gray-900">{result.header.date}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div>
              ...
              {(() => {
                ...
                if (accountingRatiosReport) {
                  ...
                  const companyName = accountingRatiosReport.company_name || accountingRatiosReport.companyName || result?.entity_name || result?.company_name || 'Company Name';
